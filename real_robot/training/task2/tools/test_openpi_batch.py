...
c=dataclasses.replace(get_config(),batch_size=2,num_workers=0,fsdp_devices=1)
...
a=tokenizer({'prompt':'Organize the fruit.','state':np.zeros(7,np.float32)})
b=tokenizer({'prompt':'Organize the fruit.','state':np.ones(7,np.float32)*.5})
