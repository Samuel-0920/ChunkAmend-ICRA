...
SESSIONS=['session_20260908_205126_777592', 'session_20260908_211413_289693', 'session_20260908_223915_362111']
NAME='uf850_fruit_organization_v001'
TASK='Organize the fruit.'
FPS=30
...
def main():
 ...
 expected=json.loads((B/'evidence/source_manifest_snapshot.json').read_text())
 if [{'path':str(p),'sha256':digest(p)} for p in sources]!=expected:raise ValueError('Source manifest snapshot changed')
 if len(sources)!=124:raise ValueError(f'Expected frozen 124 episodes, found {len(sources)}')
 ...
 for ei,mf in enumerate(sources):
  ...
  mapping.append({'episode_index':ei,'source_session':src.parents[1].name,'source_episode':src.name,'source_path':str(src),'source_manifest_sha256':digest(mf),'source_files_sha256':hashes,'samples':n,'task_success':m.get('task_success'),'success_source':m.get('success_source'),'selection_source':'user requested conversion of all 124 saved demonstrations','training':True})
  ...
  offset+=n;print(f'{ei+1}/124 converted ({offset} frames)',flush=True)
 info={'codebase_version':'v2.1','robot_type':'UF850','total_episodes':124,'total_frames':offset,'total_tasks':1,'total_videos':248,'total_chunks':1,'chunks_size':1000,'fps':FPS,'splits':{'train':'0:124'},'data_path':'data/chunk-{episode_chunk:03d}/episode_{episode_index:06d}.parquet','video_path':'videos/chunk-{episode_chunk:03d}/{video_key}/episode_{episode_index:06d}.mp4','features':features}
 ...
 write(stage/'provenance/semantics.json',{'raw_to_effective_indices':[0,1,2,3,4,5,7],'joint_unit':'radian','action':'last SDK-accepted absolute joint targets + commanded gripper pulse/850','state':'raw SDK joint payload + measured gripper pulse/850; preserved training input, not certified encoder measurement for smoothness analysis','gripper':'0 closed, 1 nominal open; measured slight overshoot retained','images':'front and wrist; no global camera; copied compressed videos unchanged','timestamp':'frame_index/30 nominal video time; actual host timestamps preserved in provenance/timing; no resampling/interpolation','hardware_synchronized':False,'success_source':'raw success labels remain unknown; all 124 selected for conversion by user, not independently verified successful','all_episodes_used_for_training':True})
 stage.rename(dest)
 write(B/'evidence/conversion_result.json',{'dataset':str(dest),'episodes':124,'frames':offset,'videos':248,'all_videos_fully_decoded':True,'raw_hashes_verified':True})
