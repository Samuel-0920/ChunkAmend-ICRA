...
case "$MODE" in
  setup)
    ...
    ;;
  validate)
    bash "$BUNDLE_ROOT/tools/verify_cpu.sh" "$BUNDLE_ROOT/tools/test_dataset_adapter.py"
    bash "$BUNDLE_ROOT/tools/verify_cpu.sh" "$BUNDLE_ROOT/tools/test_openpi_batch.py"
    ;;
  smoke|full|lora)
    ...
    CFG=pi05_uf850_fruit_organization
    EXP=pi05_uf850_fruit_organization_v001_full_s42
    EXTRA=()
    if [[ "$MODE" == lora ]]; then
      CFG=pi05_uf850_fruit_organization_lora
      EXP=pi05_uf850_fruit_organization_v001_lora_s42
    elif [[ "$MODE" == smoke ]]; then
      EXP=pi05_uf850_fruit_organization_v001_full_smoke_s42
    ...
    ;;
  *) echo '用法: bash training/server.sh setup | validate | smoke | full | lora';;
esac
