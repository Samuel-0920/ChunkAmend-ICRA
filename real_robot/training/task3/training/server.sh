...
case "$MODE" in
  ...
  smoke|full)
    ...
    CFG=pi05_uf850_task3_lemon_goblet
    EXP=pi05_uf850_task3_lemon_goblet_v001_full_s42
    EXTRA=()
    if [[ "$MODE" == smoke ]]; then
      EXP=pi05_uf850_task3_lemon_goblet_v001_full_smoke_s42
    ...
    ;;
  *) echo '用法: bash training/server.sh setup | smoke | full';;
esac
