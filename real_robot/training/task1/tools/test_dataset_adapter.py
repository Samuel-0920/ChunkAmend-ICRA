...
def main():
 ds=LeRobotDataset('local/uf850_real_lemon_to_basket_v001',root=D,
     delta_timestamps={'action':[i/30 for i in range(15)]},video_backend='pyav')
 assert len(ds)==33123 and ds.num_episodes==100
 mapping=json.loads((D/'provenance/source_map.json').read_text())
 assert len({(x['source_session'],x['source_episode']) for x in mapping})==100
 ...
 for name in ['pi05_uf850_real_lemon_to_basket','pi05_uf850_real_lemon_to_basket_lora']:
  c=config.get_config(name)
  assert c.model.discrete_state_input and c.model.action_horizon==15
  assert c.batch_size==64 and c.fsdp_devices==4 and c.data.repo_id=='local/uf850_real_lemon_to_basket_v001'
 ...
 report={'episodes':100,'frames':len(ds),'boundary_and_image_samples_checked':checked,
