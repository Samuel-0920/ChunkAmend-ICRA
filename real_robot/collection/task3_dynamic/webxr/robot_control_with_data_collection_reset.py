...
class UF850WebXRControlWithDataCollection:
    ...
    def __init__(self, robot_ip=None, dataset_path=None, preparation=None):
        ...
        print(f"旋转增益: {self.ROTATION_SCALE:g}:1")
    ...
    async def _start_episode_from_home(self):
        try:
            if os.environ.get('UF850_RELOCATION_PLAN'):
                from data_collection.relocation_plan import next_assignment
                if next_assignment(os.environ['UF850_RELOCATION_PLAN'], self.data_collector.dataset_path.parent) is None:
                    print('35条换位示教已保存；不再归位或开始新回合，请退出并审核。', flush=True)
                    return
...
async def main():
    ...
    validate_spec(load_spec(Path(__file__).resolve().parents[1]),
                  os.environ.get('UF850_TASK'), robot_ip or config.ROBOT_IP,
